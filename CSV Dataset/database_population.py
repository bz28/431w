...
cursor.execute("CREATE TABLE IF NOT EXISTS Categories (parent_category TEXT, category_name TEXT)")
...
with open("Product_Listings.csv", "r", encoding="utf-8") as file:
    ...
    for row in reader:
        seller_email, listing_id, category, product_title, product_name, product_description, quantity, product_price, status = row
        cursor.execute("INSERT INTO Product_Listings (seller_email, listing_id, category, product_title, product_name, product_description, quantity, product_price, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (seller_email, listing_id, category, product_title, product_name, product_description, quantity, product_price, status))
with open("Orders.csv", "r", encoding="utf-8") as file:
    reader = csv.reader(file)
    next(reader)

    for row in reader:
        order_id, seller_email, listing_id, buyer_email, date, quantity, payment = row
...
with open("Categories.csv", "r", encoding="utf-8") as file:
    ...
    for row in reader:
        ...
        cursor.execute("INSERT INTO Categories (parent_category, category_name) VALUES (?, ?)", (parent_category, category_name))
...
with open("Reviews.csv", "r", encoding="utf-8") as file:
    ...
    for row in reader:
        order_id, rate, review_desc = row
        cursor.execute("INSERT INTO Reviews (order_id, rate, review_desc) VALUES (?, ?, ?)", (order_id, rate, review_desc))
